		...
		.exec(function(err, registrant) {
				...
				if ((!registrant) || typeof registrant == "undefined") {
					...
					console.log("Registrant: "	+ email
									+ " not found. Creating new Registrant Schema.");
					...
					console.log("New registrant: " + email + " saved.");
					if (eventId) {
						...
						newEvent.save(function(err) {
							if (err) {
								console.log("Error saving new event for registrant " + email
														+ ". Error: " + err);
						...
						console.log("Updating registrant linked in profile.");
					    ...
					    theirProfile.save(function(err){
					    	if(err){
					    		console.log("Unable to save user linked in profile. Error: " + err);
					    		return;
					    	}
...
exports.authenticateLinkedInUser = function(req, res) {
	...
	console.log("Register via linked request recieved. "
	...
	if (!accessToken) {
		...
		path = path + '&code=' + code
				+ '&redirect_uri=http://localhost:8000/rest/linkedin/authenticate';
						...
						if ((!registrant) || typeof registrant == "undefined") {
							...
							newRegistrant
									.save(function(err) {
										if (err) {
											console
													.log("Error saving registrant. Error: "
															+ err);
											res
													.send("Error: unable to save registrant: "
															+ email + ".");
											return;
										}
									});
							...
							if (eventId) {
								...
								newEvent
										.save(function(err) {
											if (err) {
												console
														.log("Error saving new event for registrant "
																+ email
																+ ". Error: "
																+ err);
												res
														.send("Error: registrant saved, but unable to add event.");
												return;
											}
										});
